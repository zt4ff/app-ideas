...
const incrementalValue = document.getElementById(
  "increment"
) as HTMLInputElement;
...
startButton.addEventListener("click", () => {
  ...
  if (isHexa(redValue) && isHexa(greenValue) && isHexa(blueValue)) {
    ...
    setInterval(() => {
      redValue = incrementHexa(
        parseInt(redValue, 16),
        Number(incrementalValue.value) || 20
      ).toString(16);
      greenValue = incrementHexa(
        parseInt(greenValue, 16),
        Number(incrementalValue.value) || 20
      ).toString(16);
      blueValue = incrementHexa(
        parseInt(blueValue, 16),
        Number(incrementalValue.value) || 20
      ).toString(16);
